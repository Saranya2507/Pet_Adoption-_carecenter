...
w = """select*from carereg where status='new'"""
cur.execute(w)
...
<div class="sidebar d-none d-lg-block">
    ...
    <div class="content">
    <h1 style="text-align:center;color:skyblue;">New Care Center</h1>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Details for {i[1]} </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <b>Image:</b><img src="{i[1]}" style="width:80px;height:80px;"><br>
                    <b>Name:</b> {i[2]} <br>
                    <b>DOB:</b> {i[3]} <br>
...
if action and careid:
    current_time= datetime.now().strftime('%Y-%m-%d')
    if action=="accept":
        careid=form.getvalue('id')
        q="""select id from carereg where status='new'"""
        cur.execute(q)
        max = cur.fetchone()
        max_id = max[0]
        if max_id!=None:
            ...
            if final_max < 9:
                password = "CC000" + str(final_max + 1)
            elif final_max < 99:
                password = "CC00" + str(final_max + 1)
            elif final_max < 999:
                password = "CC0" + str(final_max + 1)
            else:
                password = "CC" + str(final_max + 1)
        else:
            password="CC0001"
        s=f"""select Cname,Email from carereg where id={careid}"""
        cur.execute(s)
        det = cur.fetchall()
        if det:
            name1=det[0][0]
            email=det[0][1]
            username=name1+"CC07"+name1[3:6]
            q = f"""update carereg set status='accept', careid='{username}', password='{password}',action_time='{current_time}' where id='{careid}'"""
            cur.execute(q)
            con.commit()
...
if action=="reject":
    careid = form.getvalue("id")
    q = f"""update carereg set status ='reject',action_time='{current_time}' where id='{careid}'"""
    cur.execute(q)
    con.commit()
    print("""
               <script>
