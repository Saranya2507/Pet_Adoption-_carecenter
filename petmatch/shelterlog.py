          ...
          <div class="form">
            <center>
              ...
              <h2>Shelter Login</h2>
                ...
                <p>Don't have an account?<a href="./shelterreg.py">Register</a></p>
              </form>
            </center>
          </div>
        </body>
...
if login!=None:
    q=f"""select id from shelterreg where username='%s' and password='%s'"""%(username,password)
    cur.execute(q)
    shl=cur.fetchone()
    if shl!=None:
        print(f"""
        <script>
            alert("Success");
            location.href="shelterdb.py?id={shl[0]}";
        </script>
        """)
